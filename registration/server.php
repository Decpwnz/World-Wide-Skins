...
if (isset($_POST['reg_user'])) {
  ...
  $trade = mysqli_real_escape_string($db, $_POST['trade']); // ja dodal
  ...
  if (empty($trade)) { array_push($errors, "Trade URL is required"); }
  }
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO users (username, email, password, trade) 
  			  VALUES('$username', '$email', '$password', '$trade')";
  	mysqli_query($db, $query);
  	$_SESSION['username'] = $username;
  	$_SESSION['success'] = "Zostałeś zarejestrowany!";
